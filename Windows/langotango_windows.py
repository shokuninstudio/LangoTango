...
class LangoTangoWordProcessor(QMainWindow):
    ...
    def get_icon_path(self):
        """Get correct path for app icon in both dev and binary"""
        if getattr(sys, 'frozen', False):
            # Running in PyInstaller bundle
            return os.path.join(sys._MEIPASS, "langotango.ico")
        else:
            # Running in normal Python
            return os.path.join(os.path.dirname(os.path.abspath(__file__)), "langotango.ico")
...
if __name__ == "__main__":
    ...
    # Set app icon
    icon_path = LangoTangoWordProcessor.get_icon_path(None)
    if icon_path:
        icon = QIcon(icon_path)  # Create QIcon object
